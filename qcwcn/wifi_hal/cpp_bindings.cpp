...
char *hangReasonToString(uint32_t hang_reason)
{
    ...
    C2S(QCA_WLAN_HANG_RX_HASH_NO_ENTRY_FOUND)
    C2S(QCA_WLAN_HANG_PEER_DELETION_TIMEDOUT)
    C2S(QCA_WLAN_HANG_PEER_UNMAP_TIMEDOUT)
    C2S(QCA_WLAN_HANG_SCAN_REQ_EXPIRED)
    C2S(QCA_WLAN_HANG_SCAN_ATTEMPT_FAILURES)
    C2S(QCA_WLAN_HANG_GET_MSG_BUFF_FAILURE)
    C2S(QCA_WLAN_HANG_ACTIVE_LIST_TIMEOUT)
    C2S(QCA_WLAN_HANG_SUSPEND_TIMEOUT)
    C2S(QCA_WLAN_HANG_RESUME_TIMEOUT)
    C2S(QCA_WLAN_HANG_TRANSMISSIONS_TIMEOUT)
    C2S(QCA_WLAN_HANG_TX_COMPLETE_TIMEOUT)
    C2S(QCA_WLAN_HANG_DXE_FAILURE)
    C2S(QCA_WLAN_HANG_WMI_EXCEED_MAX_PENDING_CMDS)
    C2S(QCA_WLAN_HANG_AP_STA_CONNECT_REQ_TIMEOUT)
    C2S(QCA_WLAN_HANG_STA_AP_CONNECT_REQ_TIMEOUT)
    C2S(QCA_WLAN_HANG_MAC_HW_MODE_CHANGE_TIMEOUT)
    C2S(QCA_WLAN_HANG_MAC_HW_MODE_CONFIG_TIMEOUT)
    C2S(QCA_WLAN_HANG_VDEV_START_RESPONSE_TIMED_OUT)
    C2S(QCA_WLAN_HANG_VDEV_RESTART_RESPONSE_TIMED_OUT)
    C2S(QCA_WLAN_HANG_VDEV_STOP_RESPONSE_TIMED_OUT)
    C2S(QCA_WLAN_HANG_VDEV_DELETE_RESPONSE_TIMED_OUT)
    C2S(QCA_WLAN_HANG_VDEV_PEER_DELETE_ALL_RESPONSE_TIMED_OUT)
    C2S(QCA_WLAN_HANG_WMI_BUF_SEQUENCE_MISMATCH)
    C2S(QCA_WLAN_HANG_REG_WRITE_FAILURE)
    C2S(QCA_WLAN_HANG_SUSPEND_NO_CREDIT)
    C2S(QCA_WLAN_HANG_BUS_FAILURE)
    C2S(QCA_WLAN_HANG_TASKLET_CREDIT_LATENCY_DETECT)
    C2S(QCA_WLAN_HANG_RX_MSDU_BUF_RCVD_IN_ERR_RING)
    C2S(QCA_WLAN_HANG_VDEV_SM_OUT_OF_SYNC)
    C2S(QCA_WLAN_HANG_STATS_REQ_TIMEOUT)
    C2S(QCA_WLAN_HANG_TX_DESC_LEAK)
    C2S(QCA_WLAN_HANG_SCHED_TIMEOUT)
    C2S(QCA_WLAN_HANG_SELF_PEER_DEL_FAIL)
    C2S(QCA_WLAN_HANG_DEL_SELF_STA_FAIL)
    C2S(QCA_WLAN_HANG_FLUSH_LOGS)
    C2S(QCA_WLAN_HANG_HOST_WAKEUP_REASON_PAGE_FAULT)
    default:
        return "QCA_WLAN_HANG_REASON_UNSPECIFIED";
    }
}
